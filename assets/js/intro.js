...
(function () {
  ...
  var MOTIONS = [
    'split', 'wipe', 'rise', 'drop', 'iris', 'slash',
    'push', 'diamond', 'columns', 'rows', 'fold',
    'zoom', 'sweep', 'burst', 'corner', 'shutter'
  ];
  ...
  function rememberMotion(name) {
    var rec = recentMotions().filter(function (m) { return m !== name; });
    rec.unshift(name);
    if (rec.length > 4) rec = rec.slice(0, 4);
    try { sessionStorage.setItem('jujcoRecent', JSON.stringify(rec)); } catch (e) {}
  }

  function pickMotion(prefer) {
    if (prefer && MOTIONS.indexOf(prefer) !== -1) return prefer;
    var avoid = recentMotions();
    var pool = MOTIONS.filter(function (m) { return avoid.indexOf(m) === -1; });
    if (!pool.length) pool = MOTIONS.slice();
    return pool[Math.floor(Math.random() * pool.length)];
  }

  function pageTitle() {
    var el = document.querySelector('.jdetail-hero h1, .jpage-hero h1, .cs_page_heading h1');
    if (el) {
  ...
  function pieClip(i, n) {
    ...
    function pt(a) {
      var r = a * Math.PI / 180;
      return (50 + Math.cos(r) * 80).toFixed(2) + '% ' + (50 + Math.sin(r) * 80).toFixed(2) + '%';
    }
  ...
  function buildMotion(stage, motion) {
    ...
    var n = 0;
    var kind = '';
    if (motion === 'columns') { n = 6; kind = 'cols'; }
    if (motion === 'rows') { n = 5; kind = 'rows'; }
    if (motion === 'shutter') { n = 8; kind = 'shutter'; }
    if (motion === 'burst') { n = 8; kind = 'burst'; }
  ...
  function finish() {
    ...
    blowers.forEach(function (b) { try { b.stop(); } catch (e) {} });
    if (!preloader) {
      unlock();
      return;
    }
  ...
  function runCinematic() {
    var rotor = preloader.querySelector('.jujco-cine__rotor') ||
    ...
    later(function () { preloader.classList.add('is-rev'); }, 900);
    later(function () {
      preloader.classList.add('is-title');
      blowers.forEach(function (b) { b.lock(); });
    }, 2400);
    later(finish, 5300);
  }
  ...
  function runMorph() {
    ...
    later(function () { preloader.classList.add('is-hold'); }, hold);
    later(function () { preloader.classList.add('is-open'); }, openAt);
    later(finish, outAt);
  }
  ...
  function stageHtml(motion) {
    return (
      '<div class="jtrans jtrans--exit" data-jtrans="' + motion + '" data-theme="' + themeKey() + '">' +
