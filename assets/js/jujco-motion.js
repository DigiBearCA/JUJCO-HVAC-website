...
(function () {
  ...
  function rise() {
    var els = qsa('[data-rise], .jdetail-step, .jdetail-fact, .j3d, .jstat, .jrole, .jcase, .jarticle, .jmap-card');
    if (!els.length) return;
    if (reduce || !('IntersectionObserver' in window)) {
      els.forEach(function (el) { el.classList.add('is-up'); });
      return;
    }
  ...
  function heroParallax() {
    ...
    function update() {
      ...
      heros.forEach(function (h) {
        ...
        if (orbs) orbs.style.transform = 'translate3d(0,' + (y * 0.18).toFixed(1) + 'px,0)';
      });
      ticking = false;
    }
  ...
  function hideHrefAfter() {
    ...
    s.textContent = '.jcrumbs a::after,.jcrumbs a::before,.breadcrumb a::after,.breadcrumb a::before,.jdetail-hero a::after,.cs_page_heading a::after{content:none!important;display:none!important}';
