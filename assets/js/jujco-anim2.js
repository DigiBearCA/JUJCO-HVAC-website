...
(function () {
  ...
  function themeOf(text) {
    ...
    if (/process|how we|working process|from the first/.test(t)) return 'process';
  ...
  function playWords(el, distY, distX, scale) {
    ...
    words.forEach(function (w, i) {
      ...
      w.style.transition = 'transform .55s cubic-bezier(.22,.7,.2,1), opacity .5s ease';
      w.style.transitionDelay = Math.min(i * (isSmall ? 18 : 28), 360) + 'ms';
      w.style.opacity = '0';
      w.style.transform = 'translate(' + (distX || 0) + 'px,' + (distY || 18) + 'px) scale(' + (scale || 1) + ')';
    });
  ...
  function playChars(el, theme) {
    ...
    chars.forEach(function (c, i) {
      ...
      c.style.transition = 'transform .6s cubic-bezier(.22,.7,.2,1), opacity .55s ease';
      ...
      else if (theme === 'hybrid') c.style.transform = 'translateY(14px) scale(.92)';
      else if (theme === 'emergency') c.style.transform = 'translateY(10px) scale(1.08)';
  ...
  function initHeadings() {
    if (reduceMotion) {
      qsa('.cs_hero_title, .cs_hero_mini_title, .cs_hero_subtitle, .cs_section_title, .cs_section_subtitle, .cs_page_heading h1, .cs_cta_title, .cs_accordian_title').forEach(function (el) {
        el.classList.add('is-in');
      });
      return;
    }
    ...
    qsa('.cs_section_heading p, .cs_section_heading .cs_section_text').forEach(function (el) {
      var title = el.parentElement && el.parentElement.querySelector('.cs_section_title');
    ...
    qsa('.cs_card_title, .cs_post_title, .cs_team_title, .cs_iconbox h3, .cs_iconbox_title').forEach(function (el) {
