...
(function () {
  ...
  function applyAnimVars(el) {
    ...
    if (d !== null)
      el.style.setProperty("--anim-duration", d + (/%$/.test(d) ? "" : "s"));
    if (delay !== null)
  ...
  /* ----------------------------------------------------- scroll reveal core */
  var revealSel =
    "[data-anim], [data-split], .img-fade, .reveal-mask, .reveal-wipe, " +
  ...
  var AUTO_SPLIT = [
    {
      ...
      type: "words",
      fx: ["slide", "fade", "scale", "blur"],
    },
    { sel: ".jujco-stats__num", type: "chars", fx: ["glow"], live: "glow" },
    {
      sel: ".cs_faq_question, .cs_accordion_head, .cs_accordian_head, .cs_faq_head",
      type: "words",
      fx: ["slide", "fade"],
    },
    {
      sel: ".cs_section_subtitle, .cs_subtitle",
      type: "words",
      fx: ["slide", "fade"],
    },
  ];
  var _autoIdx = {};
  function autoIndex(key, len) {
    _autoIdx[key] = _autoIdx[key] || 0;
    var i = _autoIdx[key] % len;
    _autoIdx[key]++;
    return i;
  }
  ...
  function playSplit(el) {
    ...
    var fx =
      SPLIT_FX[el.getAttribute("data-split-effect") || "fade"] || "unitFade";
    var dur = num(el.getAttribute("data-split-duration"), 500);
    var stagger = Math.min(num(el.getAttribute("data-split-stagger"), 22), 25);
    var ease =
      el.getAttribute("data-split-ease") || "cubic-bezier(0.22,0.61,0.36,1)";
    units.forEach(function (u, i) {
  ...
  function initType() {
    qsa("[data-type]").forEach(function (el) {
      ...
      cur.style.cssText =
        "display:inline-block;width:0.6em;animation:caretBlink 1s steps(1) infinite;";
      var i = 0;
      function tick() {
        if (i <= text.length) {
  ...
  function initCounters() {
    qsa("[data-count-to]").forEach(function (el) {
      ...
      function fmt(v) {
        return (
          pre + (dec ? v.toFixed(dec) : Math.round(v).toLocaleString()) + suf
        );
      }
      if (reduceMotion) {
  ...
  function initImageReveals() {
    ...
    groups.forEach(function (sel) {
      qsa(sel).forEach(function (img) {
        if (img.hasAttribute("data-anim") || img.hasAttribute("data-split"))
          return;
  ...
  function randomiseAllEffects() {
    ...
    qsa("[data-anim]").forEach(function (el) {
      if (
        !el.getAttribute("data-anim") ||
        el.getAttribute("data-anim") === "random"
      ) {
    ...
    qsa(".cs_section_heading.cs_style_1 .cs_section_title").forEach(
      function (el) {
        if (
          el.hasAttribute("data-anim") ||
          el.hasAttribute("data-split") ||
    ...
    if (!isTouch) {
      qsa(
        ".cs_service_card, .cs_post, .cs_team, .cs_project, .cs_price, .cs_award, .cs_feature, .cs_iconbox, .cs_testimonial, .cs_process, .cs_card",
      ).forEach(function (el) {
